...
public class FoToPDF {
    ...
    public File transformFoToPdf(File foFile) throws IOException {
        ...
        try(FileOutputStream fileOuputStream = new FileOutputStream(outFilePDF);
            ...
            try {
                // First try with just the base URI, no configuration
                fopFactory = FopFactory.newInstance(folderBase);
                log.info("Created FopFactory with minimal configuration to avoid event model issues");
            } catch (Exception e) {
                log.warn("Failed to create FopFactory with minimal config, falling back to full configuration: " + e.getMessage());
                // Fallback to configured version if minimal version fails
                fopFactory = FopFactory.newInstance(folderBase, isXconf);
            }
            ...
            transformer.setErrorListener(new ErrorListener() {
                ...
                public void warning(TransformerException exception) {
                    log.warn("Saxon transformer warning: " + exception.getMessage());
                }

                @Override
                public void error(TransformerException exception) {
                    log.error("Saxon transformer error: " + exception.getMessage());
                }

                @Override
                public void fatalError(TransformerException exception) {
                    log.error("Saxon transformer fatal error: " + exception.getMessage());
                }
            });
