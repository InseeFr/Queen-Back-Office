...
public class CampaignServiceImpl extends AbstractService<Campaign, String> implements CampaignService {
	...
	@Autowired
	public CampaignServiceImpl(CampaignRepository campaignRepository, MetadataRepository metadataRepository,
			QuestionnaireModelRepository questionnaireModelRepository) {
	...
	public Boolean checkIfQuestionnaireOfCampaignExists(CampaignDto campaign) {
		return campaign.getQuestionnaireIds().stream()
				.noneMatch(questionaire -> !questionnaireModelRepository.findById(questionaire).isPresent()
		...
		return campaigns.stream()
				.map(camp -> new CampaignResponseDto(
						...
						camp.getQuestionnaireModels().stream()
								.map(qm -> {
									System.out.println(qm.getValue().asText());
									return qm.getId();
								})
