...
public class SurveyUnitServiceImpl extends AbstractService<SurveyUnit, String> implements SurveyUnitService {
	...
	public void updateSurveyUnitImproved(String id, JsonNode surveyUnit) {
		...
		if (newStateData != null) {
			...
			if (!previousStateDataDate.isPresent()) {
				...
				long newDate = newStateData.get("date").asLong();
	...
	public void generateDepositProof(SurveyUnit su, HttpServletRequest request, HttpServletResponse response)
	...
	public Collection<SurveyUnitResponseDto> getSurveyUnitsByCampaign(String id, HttpServletRequest request)
		...
		for (LinkedHashMap<String, String> map : objects) {
			if (map.get("campaign").equals(id)) {
				...
				if (su.isPresent() && surveyUnitMap.get(su.get().getId()) == null) {
					...
					surveyUnitMap.put(su.get().getId(), new SurveyUnitResponseDto(su.get().getId(),
							su.get().getQuestionnaireModelId(), null, null, null, null));
				}
			}
		}
	...
	public HttpStatus postSurveyUnit(String id, SurveyUnitResponseDto su) {
		...
		Optional<QuestionnaireModel> questionnaireModelOptional = questionnaireModelService
				.findById(su.getQuestionnaireId());
		...
		if (surveyUnit.isPresent()) {
			LOGGER.info("POST survey-unit for campaign with id {} resulting in 400 : Survey-unit {} already exist", id,
					su.getId());
	...
	public ResponseEntity<String> postSurveyUnitImproved(String id, SurveyUnitResponseDto su) {
		if (!simpleApiRepository.idCampaignIsPresent(id))
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Campaign id not found");
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
					.body(String.format("Error when POST survey-unit %s for campaign %s", su.getId(), id));
		}
