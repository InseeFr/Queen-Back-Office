...
class InterrogationIT {
    ...
    void on_create_interrogation_when_campaign_not_exist_return_404() throws Exception {
        mockMvc.perform(post("/api/campaign/not-exist/survey-unit")
                        ...
                        .content(InterrogationCommonAssertions.SURVEYUNIT_DATA)
                        .with(authentication(authenticatedUserTestHelper.getAdminUser()))
                )
                .andExpect(status().isNotFound());
    }
    ...
    void on_get_survey_unit_metadata_return_survey_unit_metadata() throws Exception {
        String interrogationId = "LOG2021X11Web-01";
        MvcResult result = mockMvc.perform(get("/api/survey-unit/" + interrogationId + "/metadata")
                        .accept(MediaType.APPLICATION_JSON)
                        .with(authentication(authenticatedUserTestHelper.getAdminUser()))
                )
                .andExpect(status().isOk())
    ...
    void on_get_survey_unit_metadata_when_invalid_metadata_return_404() throws Exception {
        ...
        mockMvc.perform(get("/api/survey-unit/" + interrogationId + "/metadata")
                        .accept(MediaType.APPLICATION_JSON)
                        .with(authentication(authenticatedUserTestHelper.getAdminUser()))
                )
                .andExpect(status().isNotFound());
    ...
    void when_user_update_interrogation_with_incorrect_identifier_return_400() throws Exception {
        String interrogationDataStateData = """
            {
                "data":""" + InterrogationCommonAssertions.SURVEYUNIT_DATA + ", " +
                """
                    "stateData": {
                        "state": "EXTRACTED",
                        "date": 1111111111,
    ...
    void when_anonymous_user_access_authenticated_endpoints_return_401() throws Exception {
        List<String> getEndPoints = List.of(
                "/api/interrogations",
                "/api/interrogation/11",
                "/api/interrogation/11/deposit-proof",
                "/api/interrogation/11/metadata",
        ...
        mockMvc.perform(put("/api/interrogation/11")
                        .accept(MediaType.APPLICATION_JSON)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(InterrogationCommonAssertions.SURVEYUNIT_DATA)
                        .with(authentication(authenticatedUserTestHelper.getNotAuthenticatedUser()))
        ...
        mockMvc.perform(patch("/api/interrogation/11")
                        .accept(MediaType.APPLICATION_JSON)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(InterrogationCommonAssertions.SURVEYUNIT_DATA)
                        .with(authentication(authenticatedUserTestHelper.getNotAuthenticatedUser()))
        ...
        mockMvc.perform(delete("/api/interrogation/11")
                        .accept(MediaType.APPLICATION_JSON)
                        .with(authentication(authenticatedUserTestHelper.getNotAuthenticatedUser()))
                )
                .andExpect(status().isUnauthorized());
