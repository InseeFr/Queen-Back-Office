...
class PersonalizationIT {
    ...
    void on_update_personalization_personalization_is_updated() throws Exception {
        ...
        MvcResult result = mockMvc.perform(get("/api/survey-unit/" + interrogationId + "/personalization")
                        .accept(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(put("/api/survey-unit/" + interrogationId + "/personalization")
        ...
        result = mockMvc.perform(get("/api/survey-unit/" + interrogationId + "/personalization")
                        .accept(MediaType.APPLICATION_JSON)
                        .with(authentication(authenticatedUserTestHelper.getAdminUser()))
                )
                .andExpect(status().isOk())
    ...
    void on_update_personalization_when_personalization_not_json_array_node_return_400() throws Exception {
        mockMvc.perform(put("/api/interrogation/12/personalization")
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON)
                        .with(authentication(authenticatedUserTestHelper.getAdminUser()))
                )
    ...
    void on_get_data_when_anonymous_user_return_401() throws Exception {
        mockMvc.perform(get("/api/interrogation/12/personalization")
                        .accept(MediaType.APPLICATION_JSON)
