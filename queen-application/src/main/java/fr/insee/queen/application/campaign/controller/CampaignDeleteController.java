...
@Validated
@ConditionalOnProperty(name = "application.campaign.check-interrogations-on-delete", havingValue="true", matchIfMissing = true)
public class CampaignDeleteController {
    ...
    public void deleteCampaignById(@IdValid @PathVariable(value = "id") String campaignId) {
        if (pilotageComponent.isClosed(campaignId)) {
            campaignService.delete(campaignId, false);
